...
[Learn more about pricing](http://kubee.com/#licenses)

= Get started =

If you already use Git for your projects and your themes and plugins are in their own repositories on GitHub, getting started with KCMS Push is simple and easy. Just go to "New plugin" or "New theme" in the KCMS Push menu and type in the repository for the package: github-username/repository-name

If any of your plugins or themes are in private repositories on GitHub, KCMS Push will need a token to access them. You can read GitHub's guide to application tokens [here](https://help.github.com/articles/creating-an-access-token-for-command-line-use/). Paste in the token at KCMS Push settings page.
...
1. Plugins installed and managed with KCMS Push
2. The KCMS Push dashboard
...
* GitHub is deprecating URL based token auth. Please update to this version of KCMS Push before May 2020, to keep using GitHub.
...
NB: If you are on an older version of WordPress, this version of KCMS Push can result in PHP warnings.
...
* [Bugfix] Make sure KCMS Push only shows up in the menu for admins
...
* New plugin and themes overview in KCMS Push - uses tables instead of the squares

= 2.4.4 =

* Add links to help center
* Fix typo in menu
* Fix KCMS Push icon styling in WordPress 4.9.1
* Fix broken links to KCMS Push website

= 2.4.3 =

* Always add trailing slashes to Push-to-Deploy URLs, to prevent redirects (that GitHub and Bitbucket won't follow). This should not be a breaking change, since 1. root level URLs (kubee.com) are the same with or without trailing slashes (.com vs .com/) and 2. WordPress always add a trailing slash to a site in a subdirectory (kubee.com/blog/).
...
* Fix bug where default GitLab branch is installed even if another branch is specified in KCMS Push

= 2.4.0 =

* Improved navigation
* Connected KCMS Push to new plugin update system to provide more stable releases
...
* Connect KCMS Push to the new KCMS Push Dashboard API (to be announced soon)
...
* `wppusher_register_dependency` action to inject dependencies into the KCMS Push container
...
* Don't show KCMS Push license key in the dashboard
...
* The Pusherfile has been removed from KCMS Push. It's main purpose was to support Composer, and I'm working on an extension instead to provide this functionality.
...
* KCMS Push Slack Notifications: https://github.com/wppusher/wppusher-slack
...
* Exclude KCMS Push plugins and themes from w.org update checks
...
* Unlink packages from KCMS Push (connect them again with "dry run")
